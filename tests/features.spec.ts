...
test('assignment, structured component data and application execution lesson are available', async ({ page }) => {
  await page.goto('/', { waitUntil: 'domcontentloaded' })
  await page.getByRole('button', { name: 'ABOUT', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'PC INSIDE', exact: true })).toBeVisible()
  await expect(page.getByText('Interactive Computer Architecture Laboratory', { exact: true })).toBeVisible()
  await expect(page.getByText('Explain BIOS / UEFI', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: 'RETURN TO LAB' }).click()

  await page.locator('.component').filter({ hasText: 'CPU' }).click()
  await expect(page.locator('.info-panel')).toContainText('Fetches, decodes, and executes machine instructions')
  await expect(page.locator('.info-panel')).toContainText('8 CPU cores')
  await expect(page.locator('.info-panel')).toContainText('Instruction execution and system control')

  await page.getByRole('button', { name: 'SKIP TO RUNNING' }).click()
  await page.getByRole('button', { name: 'APPLICATION', exact: true }).click()
  await expect(page.getByTestId('execution-lesson')).toContainText('Launching a graphics application')
  await expect(page.getByTestId('execution-lesson').locator('li')).toHaveCount(5)
  await page.getByRole('button', { name: 'I/O', exact: true }).click()
  await expect(page.getByTestId('execution-lesson')).toContainText('Input and output interaction')
  await page.getByRole('button', { name: 'NETWORK', exact: true }).click()
  await expect(page.getByTestId('execution-lesson')).toContainText('Network interaction')
})

test('academic reference covers required components, concepts, and storage comparison', async ({ page }) => {
  await page.goto('/', { waitUntil: 'domcontentloaded' })
  await page.getByRole('button', { name: 'ARCHITECTURE', exact: true }).click()
  const reference = page.locator('.academic-reference')
  for (const id of ['psu', 'motherboard', 'cpu', 'ram', 'ssd', 'gpu', 'firmware', 'input-devices', 'output-devices', 'network']) {
    const card = reference.locator(`[data-reference="${id}"]`)
    await expect(card).toHaveCount(1)
    await expect(card).toContainText('WHAT IS IT?')
    await expect(card).toContainText('WHAT DOES IT DO?')
    await expect(card).toContainText('IMPORTANT CHARACTERISTICS')
    await expect(card).toContainText('COMMUNICATES WITH')
  }
  await expect(reference.locator('[data-reference="cpu"]')).toContainText('8 CPU cores / 16 threads')
  await expect(reference.locator('[data-reference="cpu"]')).toContainText('Clock speed alone does not determine performance')
  await expect(reference.locator('[data-reference="ram"]')).toContainText('6000 MT/s')
  await expect(reference.locator('[data-reference="motherboard"]')).toContainText('24-pin ATX')
  await expect(reference.locator('[data-reference="firmware"]')).toContainText('motherboard flash')
  await expect(reference.locator('[data-reference="input-devices"]')).toContainText('KEYBOARD / MOUSE / MICROPHONE')
  await expect(reference.locator('[data-reference="output-devices"]')).toContainText('DISPLAY OUTPUT')
  await expect(reference.locator('[data-reference="network"]')).toContainText('ETHERNET / WI-FI')
  await expect(reference.locator('.comparison-row')).toHaveCount(4)
  await expect(reference).toContainText('HDD')
  await expect(reference).toContainText('SATA SSD')
  await expect(reference).toContainText('NVMe SSD')
  await expect(reference).toContainText('Values are illustrative typical ranges')
  await expect(reference).toContainText('Example hardware specifications are illustrative')
})

test('running controls stay compact and do not overlap the view switch', async ({ page }) => {
  await page.goto('/', { waitUntil: 'domcontentloaded' })
  await page.getByRole('button', { name: 'SKIP TO RUNNING' }).click()

  const processSelector = await page.locator('.process-selector').boundingBox()
  const viewSwitch = await page.locator('.view-switch').boundingBox()
  const bootController = await page.locator('.boot-console').boundingBox()

  expect(processSelector).not.toBeNull()
  expect(viewSwitch).not.toBeNull()
  expect(bootController).not.toBeNull()
  expect(processSelector!.x + processSelector!.width).toBeLessThanOrEqual(viewSwitch!.x - 8)
  expect(bootController!.height).toBeLessThan(125)
  await expect(page.locator('.boot-log')).toBeHidden()

  await page.setViewportSize({ width: 1180, height: 800 })
  const mediumProcessSelector = await page.locator('.process-selector').boundingBox()
  const mediumViewSwitch = await page.locator('.view-switch').boundingBox()
  expect(mediumProcessSelector!.x + mediumProcessSelector!.width).toBeLessThanOrEqual(mediumViewSwitch!.x - 8)
})
